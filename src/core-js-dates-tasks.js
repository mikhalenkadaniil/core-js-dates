 ...
 */
function getCountDaysOnPeriod(ISOStart, ISOEnd) {
  const dateStart = new Date(ISOStart);
  const dateEnd = new Date(ISOEnd);
  return (dateEnd - dateStart) / (1000 * 60 * 60 * 24) + 1;
}
...
function isDateInPeriod(date, period) {
  if (
    new Date(date) - new Date(period.start) >= 0 &&
    new Date(date) - new Date(period.end) < 0
  ) {
    return true;
  }
  return false;
...
function formatDate(inputDate) {
  const [date, args] = inputDate.split('T');
  const time = args.split('.')[0];
  ...
  return `${+splittedDate[1]}/${+splittedDate[2]}/${+splittedDate[0]}, ${+splittedTime[0]}:${splittedTime[1]}:${splittedTime[2]} ${part}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= getCountDaysInMonth(month, year); i += 1) {
    const day = new Date(year, jsMonth, i, 12);
...
function getWeekNumberByDate(date) {
  ...
  let i = firstDay;
  for (i = firstDay; i < 7; i += 1) {
    if (month === 0 && day === new Date(year, 0, i - firstDay + 1).getDate()) {
      return 1;
    }
  }
  while (new Date(year, 0, i, 12) - date < 0) {
    i += 7;
    week += 1;
  }
  return week;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (date <= endDate) {
    if (workDaysLeft === 0) {
      ...
      date.setDate(date.getDate() + countOffDays - 1);
    } else {
      result.push(formatDateHere(new Date(date)));
      workDaysLeft -= 1;
    }
